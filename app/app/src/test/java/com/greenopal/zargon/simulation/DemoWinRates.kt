...
class DemoWinRates {
    ...
    fun `demo win rates for level 5 character`() {
        ...
        println("Player Stats: Level ${character.level}, AP=${character.totalAP}, HP=${character.currentHP}, Defense=${character.totalDefense}")
        println()
    ...
    fun `demo win rates across levels vs NECRO`() {
        ...
        for (scale in listOf(1, 2, 5, 10)) {
            ...
            for (level in listOf(1, 3, 5, 7, 10, 15)) {
                ...
                val turnsStr = if (avgTurns.isNaN()) "N/A" else String.format("%.1f", avgTurns)
                val statsStr = "AP=${character.totalAP.toString().padEnd(2)} HP=${character.currentHP.toString().padEnd(3)}"
